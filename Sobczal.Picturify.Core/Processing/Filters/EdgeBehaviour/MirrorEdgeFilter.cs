...
{
    public class MirrorEdgeFilter : BaseFilter
    {
                private readonly PSize _range;

        public MirrorEdgeFilter(PSize range)
        {
            _range = range;
        }
        ...
        private float[,,] BeforeProcessingFunctionF(float[,,] pixels, CancellationToken cancellationToken)
        {
            ...
            for (var i = 0; i < rangeX; i++)
            {
                for (var j = 0; j < rangeY; j++)
                {
                    ...
                    {
                        arr[i, j, k] = pixels[rangeX - i - 1, rangeY - j - 1, k];
                        arr[i, heightArr - j - 1, k] = pixels[rangeX - i - 1, heightPx - rangeY + j, k];
                        arr[widthArr - i - 1, j, k] = pixels[widthPx - rangeX + i, rangeY - j - 1, k];
                        arr[widthArr - i - 1, heightArr - j - 1, k] = pixels[widthPx - rangeX + i, heightPx - rangeY + j, k];
                    }
                }
            }
            ...
            for (var i = rangeX; i < widthArr - rangeX; i++)
            {
                for (var j = 0; j < rangeY; j++)
                {
                    for (var k = 0; k < depthArr; k++)
                    {
                        arr[i, j, k] = pixels[i - rangeX, rangeY - j - 1, k];
                        arr[i, heightArr - 1 - j, k] = pixels[i - rangeX, heightPx - rangeY + j, k];
                    }
                }
            }
            ...
            for (var i = 0; i < rangeX; i++)
            {
                for (var j = rangeY; j < heightArr - rangeY; j++)
                {
                    for (var k = 0; k < depthArr; k++)
                    {
                        arr[i, j, k] = pixels[rangeX - i - 1, j - rangeY, k];
                        arr[widthArr - i - 1, j, k] = pixels[widthPx - rangeX + i, j - rangeY, k];
                    }
                }
            }
            sw.Stop();
